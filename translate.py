    ...
    for i in range(len(parsed_f)):
        ...
        if re.match(r"^(\s*)input\s+(\[[0-9:]+\][\w\d\s,]+)+;$", parsed_f[i]):
            ...
            for j in range(len(x)):
                ...
                for k in range(int(y[2]), int(y[1]) + 1):
                    ...
                    # the following code creates constant gates in the circuit graph. This is unnecessary and creted only once in Initial state
                    # if constant_gate == "":
        ...
        if re.match(r"^(\s*)input\s+([\w\d\s,\[\]]+);$", parsed_f[i]):
            ...
            for j in range(len(x)):
                ...
                # Construct Constant Gates
                # #####################################
                # the following code creates constant gates in the circuit graph. 
                ...
                # Construct a buffer gate between reset node and constant_0
            continue
        ...
        #####################################
        # if re.match(r"^(\s*)DFF_[\w\d\s]*\([\w\d\s,\(\)\[\]\.]+\);$", parsed_f[i]):
        strTemp = '^([\s\S]*)'+'DFF'+'[\w\d\s\S]*\([\w\d\s\S,\(\)\[\]\.]+\);$' # r"^([\s\S]*)DFF[\w\d\s\S]*\([\w\d\s\S,\(\)\[\]\.]+\);$"
        if re.match(strTemp, parsed_f[i]):
            #####################################
            # Match D Port
            #####################################
            D = re.search(r"\.D\([\w\d\s\S\[\]]+\)", parsed_f[i])
            if (D):
                # D_port = re.split(r"\(|\)", D.group())[1]
                D_port = re.split(r"\(|\)", D.group())[1].replace(" ", "")
                if (D_port) in input_list:
                    input_visit[input_list.index(D_port)] = 1
            else:
                exit("D port doesn't exist!")
            ...
            # Match Q Port
            #####################################
            Q = re.search(r"\.Q\([\w\d\s\S\[\]]+\)", parsed_f[i])
            Q_port = ""
            if (Q):
                Q_port = re.split(r"\(|\)", Q.group())[1].replace(" ", "")
                if (D_port in flipflop_D_ports):
        ...
        #####################################
        if re.match(r"^(\s*)([\w\d]+)(\s+)([\w\d]+)(\s+)\([\w\d\s\S,\(\)\[\]\.]+\);$", parsed_f[i]):
            ...
            if ("INV_" in gate_type) or ("BUF_" in gate_type):
                ...
                A = re.search(r"\.(A)*(I)*\([\w\d\s\S\[\]]+\)", parsed_f[i])
                if (A):
            ...
            #####################################
                ...
                #####################################
                # A = re.search(r"\.A\([\w\d\s\[\]]+\)", parsed_f[i])
                A = re.search(r"\.(A)*(A1)*\([\w\d\s\S\[\]]+\)", parsed_f[i])
                if (A):
                    A_port = re.split(r"\(|\)", A.group())[1].replace(" ", "")
                    if (A_port) in input_list:
                        input_visit[input_list.index(A_port)] = 1
                ...
                B = re.search(r"\.(B)*(A2)*\([\w\d\s\S\[\]]+\)", parsed_f[i])
                if (B):
            ...
            #####################################
                ...
                A1 = re.search(r"\.(A1)*(I0)*\([\w\d\s\S\[\]]+\)", parsed_f[i])
                if (A1):
                ...
                A2 = re.search(r"\.(A2)*(I1)*\([\w\d\s\S\[\]]+\)", parsed_f[i])
                if (A2):
                ...
                # Match A3 Port
                #####################################
                A3 = re.search(r"\.(A3)*(I0)*\([\w\d\s\S\[\]]+\)", parsed_f[i])
                if (A3):
                    A3_port = re.split(r"\(|\)", A3.group())[1].replace(" ", "")
                    if (A3_port) in input_list:
                        input_visit[input_list.index(A3_port)] = 1
                # else:
                #     exit("A1 port doesn't exist!")
                ...
                # Match A4 Port
                #####################################
                A4 = re.search(r"\.(A4)*(I0)*\([\w\d\s\S\[\]]+\)", parsed_f[i])
                if (A4):
                    A4_port = re.split(r"\(|\)", A4.group())[1].replace(" ", "")
                    if (A4_port) in input_list:
                        input_visit[input_list.index(A4_port)] = 1
                # else:
                #     exit("A1 port doesn't exist!")
                ...
                # Match Z/ZN Port
                #####################################
                Z = re.search(r"\.Z(N?)\([\w\d\s\S\[\]]+\)", parsed_f[i])
                # if (Z):
                #     Z_port = re.split(r"\(|\)", Z.group())[1].replace(" ", "")
                #     of.write(Z_port + " = " + gate_type.split("2_")[0] + "(" + A1_port + ", " + A2_port + ")\n")
                if(Z):
                    if (Z and A3 is None and A4 is None):
                        Z_port = re.split(r"\(|\)", Z.group())[1].replace(" ", "")
                        of.write(Z_port + " = " + gate_type.split("2_")[0] + "(" + A1_port + ", " + A2_port + ")\n")
                    if (Z and A3 and A4 is None):
                        Z_port = re.split(r"\(|\)", Z.group())[1].replace(" ", "")
                        of.write(Z_port + " = " + gate_type.split("2_")[0] + "(" + A1_port + ", " + A2_port + ", " + A3_port + ")\n")
                    if (Z and A3 and A4):
                        Z_port = re.split(r"\(|\)", Z.group())[1].replace(" ", "")
                        of.write(Z_port + " = " + gate_type.split("2_")[0] + "(" + A1_port + ", " + A2_port + ", " + A3_port + ", " + A4_port + ")\n")
                else:
                    exit("Z/ZN port doesn't exist!")
        ...
        if node.gate_type != bench_net.gateType['IPT']:
            if node.gate_type != bench_net.gateType['DFF']:
                ...
                if(not isinstance(node.fan_in_node[0],str)):
                    ...
                    fout.write('dff AbC_%d(clk, %s, %s);\n' % (gate_count, node.name, node.fan_in_node[0]))
